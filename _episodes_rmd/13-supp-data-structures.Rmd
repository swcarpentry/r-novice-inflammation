...
dim(m)
```
You can create a matrix with single element (say 'x') following way
...
dim(m)
```
A diagonal matrix with element 'x' can be created using 'diag'command

```{r}
m <- diag(x, nrow = 4, ncol = 4)
m
dim(m)
```
