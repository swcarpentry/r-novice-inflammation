...
```{r echo = FALSE}
tmp <- read.csv("data/sample-01.csv", header = TRUE, nrows = 5)
...
```{r, results="hide"}
read.csv(file = "data/sample-01.csv", header = TRUE)
```
...
Since we didn't tell it to do anything else with the function's output, the console will display the full contents of the file `sample-01.csv`.
Try it out.
...
`plot` created a scatter plot where the y-axis is the average inflammation level and the x-axis is the order, or index, of the values in the vector, which in this case correspond to the 4 quadrants of the eye.
...
Our work so far has looked at the first data sample, we have 4 more from subsequent years.
