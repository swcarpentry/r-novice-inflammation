...
dat[1, 1]
# Get the value in the 30th row and 20th column
dat[30, 20]
```

Using this approach, we can select specific values in a data.frame. We can also select the first ten days (columns) of values for the first four patients (rows) like this:
...
To select non-contiguous values, we can use the function `c`, which stands for **c**ombine.
...
```{r}
# Values from row 5 and all columns
dat[5, ]
# Values from alls rows for column 16
dat[, 16]
```

We can also remove rows and columns using the negative sign.

Recall that our data frame, `dat`, has `r nrow(dat)` rows and `r ncol(dat)` columns.
...
Now the object `a5`, has `r nrow(a5)` rows and `r ncol(a5)` columns. Nothing has been removed from our original data.

For columns, we can use the same approach:
```{r}
#remove 16th column
a16<-dat[, -16]
```

Now the object `a16`, has `r nrow(a16)` rows and `r ncol(a16)` columns.

Similiarly, if we want to remove multiple rows and columns, just add the engative sign to the c() vector we used before.

```{r}
a<-dat[-c(3, 8, 37, 56), -c(10, 14, 29)]
dim(a)
```
