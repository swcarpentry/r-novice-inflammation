...
> ## Challenge - Slicing and re-assignment {.challenge}
> 
...
> 2. Halve the first five days' values in only those patients
> 3. Print out the data frame to check that your code has fixed the problem
>
>> Solution:
>> ```{r}
>> whichPatients <- seq(2,40,2)
>> whichDays <- c(1:5)
>> dat[whichPatients,whichDays] <- dat[whichPatients,whichDays]/2
>> (dat)
>> ```
>
