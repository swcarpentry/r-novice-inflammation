...
dat <- read.csv(file = "data/sample-01.csv", header = TRUE)
...
```{r}
diff <- read.csv(file = "data/sample-01.csv", header = TRUE)
...
```{r}
dat <- read.csv(file = "data/sample-01.csv", header = FALSE)
centered <- center(dat[, 5], 0)
head(centered)
```
...
```{r}
all.equal(sd(dat[, 5]), sd(centered))
```
...
#### Challenges
  ...
  + Write a function called `analyze` that takes a filename as a argument and displays the three graphs produced in the [previous lesson][01] (average, min and max aneurism count of each quadrant).
  `analyze("data/sample-01.csv")` should produce the graphs already shown, while `analyze("data/sample-02.csv")` should produce corresponding graphs for the second data set. Be sure to document your function with comments.
...
analyze <- function(filename) {
  ...
  avg_quad_aneurism <- apply(dat[,5:8], 2, mean)
  plot(avg_quad_aneurism)
  max_quad_aneurism <- apply(dat[,5:8], 2, max)
  plot(max_quad_aneurism)
  min_quad_aneurism <- apply(dat[,5:8], 2, min)
  plot(min_quad_aneurism)
}
```
...
answer <- rescale(dat[, 5])
min(answer)
max(answer)
plot(answer)
plot(dat[, 5], answer)  # This hasn't been introduced yet, but it may be
                        # useful to show when explaining the answer.
```

### Defining Defaults

We have passed arguments to functions in two ways: directly, as in `dim(dat)`, and by name, as in `read.csv(file = "data/sample-01.csv", header = FALSE)`.
In fact, we can pass the arguments to `read.csv` without naming them:

```{r}
dat <- read.csv("data/sample-01.csv", TRUE)
```

However, the position of the arguments matters if they are not named.

```{r}
dat <- read.csv(header = TRUE, file = "data/sample-01.csv")
dat <- read.csv(TRUE, "data/sample-01.csv")
```
...
```{r, results="hide"}
dat <- read.csv(TRUE, "data/inflammation-01.csv")
```
...
answer <- rescale(dat[, 5], lower = 2, upper = 5)
min(answer)
max(answer)
answer <- rescale(dat[, 5], lower = -5, upper = -2)
min(answer)
max(answer)
```
...
```{r, eval=FALSE}
analyze("data/sample-01.csv")
analyze("data/sample-02.csv")
#...
analyze("data/sample-05.csv")
```
