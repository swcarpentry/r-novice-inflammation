...
> ## Sarah's Challenge - Another function {.challenge}
>
>  Below is an incomplete function that calculates some basic statistical descriptions of a vector, `x`. Add to the function so that it automatically creates a dataframe output that includes each of the variables calculated by the function. Be sure to give columns of the dataframe descriptive names. Use the `round` function to round each result of the output to 3 places.
>  Be sure to document each step of the function with comments.

```{r rescale-test, include=TRUE}
describe = function(x)
+ {
+    m <- mean(x,na.rm=T)
+    s <- sd(x,na.rm=T)
