...
The data sets are stored in [comma-separated values](http://software-carpentry.org/v5/gloss.html#csv) (CSV) format: each row holds information for a single patient, and the columns represent successive days. 
...
The expression `read.csv(...)` is a [function call](http://software-carpentry.org/v5/gloss.html#function-call) that asks R to run the function `read.csv`. 

`read.csv` has two [arguments](http://software-carpentry.org/v5/gloss.html#argument): the name of the file we want to read, and whether the first line of the file contains names for the columns of data.
The filename needs to be a character string (or [string](http://software-carpentry.org/v5/gloss.html#string) for short), so we put it in quotes.
...
We can see the dimensions, or [shape](http://software-carpentry.org/v5/gloss.html#shape), of the data frame like this:
...
If we want to get a single value from the data frame, we can provide an [index](http://software-carpentry.org/v5/gloss.html#index) in square brackets, just as we do in math:
...
The [slice](http://software-carpentry.org/v5/gloss.html#slice) `1:4` means, "Start at index 1 and go to index 4."
...
> ## Challenge {.challenge}
>
> A subsection of a data frame is called a [slice](http://software-carpentry.org/v5/gloss.html#slice).
