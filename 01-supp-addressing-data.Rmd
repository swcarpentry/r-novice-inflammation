...
Data can also be accessed by **name**, as columns in an R data frame are named.
...
We can also use **logical indexing**. A logical vector contains only the special values `TRUE` & `FALSE`.
...
Finally, the assignment operator `<-` can be **combined** with indexing.
