...
Since we did not specify a filename for writing, `cat` sends the output to [standard output]({{ site.github.url }}/reference/#standard-output-(stdout)),
...
>
>    **Hint:** Everything argument read from the command-line is interpreted as a character [string]({{ site.github.url }}/reference/#string).
...
>
> 3. Using the function `list.files` introduced in a previous [lesson]({{ site.github.url }}/03-loops-R/),
...
2.  If `action` isn't one of the three recognized flags, the program loads each file but does nothing with it (because none of the branches in the conditional match).
    [Silent failures]({{ site.github.url }}/reference/#silent-failure) like this are always hard to debug.
