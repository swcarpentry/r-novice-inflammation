...
Next, the [body](http://software-carpentry.org/v5/gloss.html#function-body) of the function--the statements that are executed when it runs--is contained within curly braces (`{}`).
...
Inside the function, we use a [return statement](http://software-carpentry.org/v5/gloss.html#return-statement) to send a result back to whoever asked for it.
...
Instead, we can [compose](http://software-carpentry.org/v5/gloss.html#function-composition) the two functions we have already created:
...
Instead, it creates something called a [stack frame](http://software-carpentry.org/v5/gloss.html#stack-frame) to keep track of the variables defined by `fahr_to_kelvin`.
...
The big idea here is [encapsulation](http://software-carpentry.org/v5/gloss.html#encapsulation), and it's the key to writing correct, comprehensible programs.
...
We have one more task first, though: we should write some [documentation](http://software-carpentry.org/v5/gloss.html#documentation) for our function to remind ourselves later what it's for and how to use it.

A common way to put documentation in software is to add [comments](http://software-carpentry.org/v5/gloss.html#comment) like this:
...
* Each time a function is called, a new stack frame is created on the [call stack](http://software-carpentry.org/v5/gloss.html#call-stack) to hold its arguments and local variables.
